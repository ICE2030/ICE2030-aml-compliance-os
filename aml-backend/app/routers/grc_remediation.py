...
@router.get("/summary")
async def get_remediation_summary(db: AsyncSession = Depends(get_db)):
...
async def update_action(action_id: str, data: dict, db: AsyncSession = Depends(get_db)):
...
async def create_milestone(action_id: str, data: dict, db: AsyncSession = Depends(get_db)):
...
async def list_milestones(action_id: str, db: AsyncSession = Depends(get_db)):
    """List milestones for a remediation action."""
    return await RemediationService.list_milestones(db, action_id)


@router.put("/milestones/{milestone_id}")
async def update_milestone(milestone_id: str, data: dict, db: AsyncSession = Depends(get_db)):
    """Update a milestone."""
